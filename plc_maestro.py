...
La banda transportadora NO esta aqui: es un PLC independiente con su
propio Ladder maestro y su propio mapa de registros (ver plc_banda.py).
============================================================================
"""
...
class XL4:
    ...
    def habilitar(self, on=True):
        # El Ladder maestro del maletin es standalone: MasterEnable := CmdWord
        # <> 0, sin selector de equipo. Ya NO se escribe SysMode (%R5): la
        # banda tiene su propio PLC (plc_banda.py) y ese registro no existe en
        # este programa.
    ...
    def reset_todo(self, borrar_acumulados=True):
        ...
        # Aqui NO se tocan SysMode (%R5) ni BandCmdWord (%R90): pertenecian al
        # ladder viejo, cuando banda y maletin compartian PLC. Hoy la banda es
        # un equipo aparte y este reset solo debe alcanzar registros del
        # Ladder maestro del maletin.
...
def validar_config(cfg) -> list:
    ...
    if not isinstance(outputs, list):
        outputs = []
    # SEPARACION DE EQUIPOS: la banda transportadora es un PLC INDEPENDIENTE,
    # con su propio Ladder maestro (Programa_Banda.txt) y su propio mapa de
    # registros. Su bloque 'band' se valida y se escribe en plc_banda.py; aqui
    # se rechaza para que ningun registro de un equipo llegue al otro.
    if band is not None:
        return ["El bloque 'band' pertenece al PLC de la banda transportadora. "
                "Cargalo con plc_banda.py (device='banda'), no en el maletin."]
    ...
    # I3/I4 vuelven a ser entradas normales del maletin: los sensores S1/S2
    # estan en el PLC de la banda, que es otro equipo con otras entradas.
...
def plan_config(cfg) -> list:
    ...
    # La banda NO se planifica aqui: es otro PLC (ver plc_banda.plan_config).
    # validar_config() ya rechaza cualquier cfg que traiga el bloque 'band'.
